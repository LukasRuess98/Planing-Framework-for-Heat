...
def simple_config() -> dict:
    return {
        ...
        "costs": {
            ...
            "co2_price_eur_per_t": 42.0,
            "dump_cost_eur_per_mwh_th": 1.5,
...
def test_rh_only_workflow_aggregates(monkeypatch: pytest.MonkeyPatch, simple_config: dict) -> None:
    ...
    demand_total = sum(table["waermebedarf_MWth"])
    ...
    assert costs["objective.Include_CO2_in_objective"] is False
    assert costs["objective.CO2_price_EUR_per_t"] == pytest.approx(42.0)
    assert costs["objective.P_buy_peak_MW"] == pytest.approx(3.0)
    assert costs["objective.Grid_energy_cost_EUR"] == pytest.approx(0.0)
    assert costs["objective.Grid_sell_revenue_EUR"] == pytest.approx(0.0)
    assert costs["objective.Grid_net_cost_EUR"] == pytest.approx(0.0)
    assert costs["objective.OBJ_value_EUR"] == pytest.approx(0.0)
    assert costs["objective.Demand_charge_cost_EUR"] == pytest.approx(0.0)
    assert costs["grid.Energy_from_grid_MWh"] == pytest.approx(9.0)
    assert costs["grid.Net_grid_import_MWh"] == pytest.approx(9.0)
    assert costs["grid.Heat_demand_MWh"] == pytest.approx(demand_total)
...
def test_pf_then_rh_fix_design(monkeypatch: pytest.MonkeyPatch, simple_config: dict) -> None:
    ...
    costs = result.rh_result.costs
    assert costs["objective.P_buy_peak_MW"] == pytest.approx(3.0)
    assert costs["heat_pump_HP1.Thermal_capacity_MW"] == pytest.approx(5.0)
    bounds = costs["heat_pump_HP1.Capacity_bounds_MW"]
    assert bounds[0] == pytest.approx(5.0)
    assert bounds[1] == pytest.approx(5.0)
    assert costs["heat_pump_HP1.Investment_enabled"] is False
    assert costs["storage_TES.Capacity_MWh"] == pytest.approx(20.0)
    assert costs["storage_TES.Power_limit_MW"] == pytest.approx(5.0)
    assert costs["storage_TES.Build_binary"] == pytest.approx(1.0)
